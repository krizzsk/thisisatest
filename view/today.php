 ...
 <div style="overflow: <?php echo(!$print)?'auto':'hidden';?>;padding:2%;margin:2%'; ?>" id="detailDiv" dojoType="dijit.layout.ContentPane" region="center">
      ...
      <div style="width:0px;height:0px;">
        <button id="todayParametersButton" dojoType="dijit.form.Button"
          ...
          style="position:absolute;right:45px;width: 28px;top:15px;" class="detailButton"
    ...
    <?php  }
      $today=new Today();
      $crit=array('idUser'=>$user->id, 'idle'=>'0','scope'=>'report');
      $todayList=$today->getSqlElementsFromCriteria($crit, false, null, 'sortOrder asc');
      $i = 7;
      foreach ($todayList as $todayR){
        $i++;
        $rpt=new Report($todayR->idReport);
      ?>
      <div id="reportToday<?php echo $i;?>" class="simple-grid__cell simple-grid__cell--1-4" style="order:<?php echo $i;?>">
        <div class="simple-grid__container" style="min-height:40px;">
        <div ></div>
        <?php         
        ...
        if (!$print) {
          if (!$print) {
            ...
            echo ' onMouseOver="dojo.byId(\'deleteReport'.$todayR->id.'\').style.display=\'block\';dojo.byId(\'hideReport'.$todayR->id.'\').style.display=\'block\';" onMouseOut="dojo.byId(\'deleteReport'.$todayR->id.'\').style.display=\'none\';dojo.byId(\'hideReport'.$todayR->id.'\').style.display=\'none\';" > ';
            ...
            $image='../view/css/images/tabClose.svg';
            echo '<img class="roundedButtonSmall  iconSize22" src="'.$image.'" onClick="hideDivToday(\'reportToday'.$i.'\');setTodayParameterHiderReportSection('.$todayR->id.');" />';
            echo "</div>";
            ...
            echo '<img class="roundedButtonSmall imageColorNewGui   iconSize22" src="'.$image.'" onClick="hideDivToday(\'reportToday'.$i.'\');setTodayParameterDeleteReport('.$todayR->id.');" />';
            echo "</div>";
            echo "</div>";
            echo '<div  dojoType="dijit.TitlePane" style="overflow-x:auto;border:0"';
           // echo ' onMouseOver="dojo.byId(\'hideReport'.$todayR->id.'\').style.display=\'block\';" onMouseOut="dojo.byId(\'hideReport'.$todayR->id.'\').style.display=\'none\';"  ';
